...
import {
  ...
  AlertCircle, CheckCircle, ExternalLink, Clock, Eye, Rss, Plus, Trash2,
} from "lucide-react";
...
// ── Channels Feed ─────────────────────────────────────────────────────────────
function ChannelsFeed({ onSelectVideo }: { onSelectVideo: (video: Video) => void }) {
...
export default function Home() {
  const [tab, setTab] = useState<"search" | "feed">("search");
    ...
    <div className="min-h-screen bg-background flex flex-col font-sans">
      ...
      <header className="sticky top-0 z-40 border-b border-border glass-panel">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 h-16 flex items-center gap-4">
          ...
          {/* Tabs */}
          <div className="flex items-center gap-1 p-1 rounded-xl bg-background border border-border shrink-0">
            <button
              onClick={() => setTab("search")}
              className={`flex items-center gap-1.5 px-3 h-8 rounded-lg text-sm font-semibold transition-colors ${
                tab === "search" ? "bg-primary text-white" : "text-text-muted hover:text-text-main"
              }`}
            >
            ...
            <button
              onClick={() => setTab("feed")}
              className={`flex items-center gap-1.5 px-3 h-8 rounded-lg text-sm font-semibold transition-colors ${
                tab === "feed" ? "bg-primary text-white" : "text-text-muted hover:text-text-main"
              }`}
            >
      ...
      <main className="flex-1 max-w-[1400px] w-full mx-auto px-4 sm:px-6 py-8">
        ...
        {tab === "feed" && <ChannelsFeed onSelectVideo={setSelectedVideo} />}
